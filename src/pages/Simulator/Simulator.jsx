...
const Simulator = () => {
    ...
    useEffect(() => {
        const cuotasIniciales = ownersData.map(owner => ({
            ...owner,
            nuevaCuota: 0
            // nuevaCuota: owner.cuota_basica // Inicialmente la cuota estimada es igual a la cuota básica
        }));
    ...
    // const calcularCuotas = (nuevoGasto) => {
    //     const totalGastos = gastosAdicionales.reduce((total, gasto) => total + gasto.monto, 0) + nuevoGasto.monto;
    //     const cuotasActualizadas = ownersData.map(owner => {
    //         const nuevaCuota = owner.cuota_basica + (totalGastos * owner.coeficiente);
    //         return { ...owner, nuevaCuota };
    //     });
    //     setCuotasCalculadas(cuotasActualizadas);
    // };
